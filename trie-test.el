(ert-deftest trie-verify-trie-against-linux-kernel-compile-commands-json ()
  ...
  (let ((filename-list (list))
        (filename-trie (add-string-to-trie nil " " nil 0)))
    (with-temp-buffer
      ...
      (while (not (eobp))
        ...
        (add-string-to-trie filename-trie (match-string 0) nil 0)
        (forward-line 1)))
    (mapcar (lambda (fn)
...
(ert-deftest trie-verify-prefixes-are-found ()
  ...
  (let ((test-trie (add-string-to-trie nil " " nil 0)))
    ...
    (add-string-to-trie test-trie "/main" nil 0)
    (add-string-to-trie test-trie "/version.c" nil 0)
    (should (equal
